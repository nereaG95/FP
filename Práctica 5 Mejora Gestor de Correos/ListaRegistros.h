...
typedef tRegCorreo *tPuntRegistro;
//typedef tRegCorreo tLista[MAX_REGISTROS];

typedef struct {
	tPuntRegistro registros;
	int cont;
	int capacidadActual;
}tListaRegCorreo;
...
/*Funcion que dado un registro lo inserta en la lista de registros si esta no está llena. Esta funcion inserta el registro al final de la lista.
...
/*
Es un procedimiento que destruye del monton los datos a los que apunta el array dinamico liberando la memoria del monton y poniendo el contador a 0
E/S: listaReg(tListaRegCorreo) es la lista que queremos borrar
*/
