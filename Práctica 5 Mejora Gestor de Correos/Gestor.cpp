...
/*
Es un procedimiento que solicita al usuario un id y una contrase?a, le vuelve a pedir la contrase?a y si no coincide muestra
un error y vuelve a solicitar todo de nuevo
S: id(string) y contra(string) son los datos que solicita al usuario y los que devuelve la funcion
*/
...
void solicitarDatosNuevaCuenta(string &id , string &contra){
	...
	do{
		...
		cout << "Introduce de nuevo la contrase?a :";
		cin >> contra2;
		if(contra!=contra2){
			cout <<"Las contrase?as no coinciden, no se ha podido crear la cuenta" << endl;
			system("pause");
			system("cls");
		}
...
bool iniciarSesion(tGestor &gestor){
	...
	if(buscarUsuario(gestor.listaUsuarios, id, pos)){
		...
		if (validarContrasenia(usuario,contra)){ //si la contrase?a es correcta y el usuario se encuentra en la lista
			gestor.usuarioActivo = pos; 
			ok = true;
		}
	}
	if(!ok) //No se encuentra el usuario o la contrase?a es incorrecta
...
void borrarCorreo(tGestor& gestor, tListaRegCorreo& listaReg){
	...
	if(solicitarPos(listaReg,pos)){
		...
		buscar(gestor.listaCorreos,id, pos);
		}
	else{
...
void ejecutarMenuBandeja(tGestor &gestor, bool &entrada, int op){
	...
	case 2:
		...
		if(!enviarCorreo(gestor,correo)){
			cout << "No se ha podido enviar el correo,porque el destinatario no existe! " << endl;
		}
		system("pause");
		break;
	case 3:
...
bool enviarCorreo(tGestor& gestor, const tCorreo &correo){
	...
	int pos;
	bool ok=false;
	insertar(gestor.listaCorreos, correo); //Insertamos el correo en la lista de correos del gestor
	regUsuarioActivo.id = correo.id;
	regUsuarioActivo.leido = true;
	insertar(gestor.listaUsuarios.usuario[gestor.usuarioActivo]->enviados,regUsuarioActivo);
	regUsuarioActivo.leido = false;
	if(buscarUsuario(gestor.listaUsuarios, correo.destinatario, pos)){
		ok=true;
		insertar(gestor.listaUsuarios.usuario[pos]->recibidos, regUsuarioActivo);
	}
	return ok;
}
